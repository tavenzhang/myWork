...
public class baseScroll_taven extends  EventDispatcher {
	...
	private var _scaleMinValue:int = 10;//缩放最小值
	private var _initH:Number = 0;//原始高
	private var _nowH:Number = 0;//当前高
	private var _containView:Sprite;

	public function  get view():Sprite
	{
		return _containView;
	}
	public function baseScroll_taven(contentView:Sprite,thumb:Sprite = null, track:Sprite = null, rect:Rectangle = null, upButton:SimpleButton = null, downButton:SimpleButton = null):void {
		this.formatBaseScroll(contentView,thumb, track, rect, upButton, downButton);
	}

	protected function formatBaseScroll(contentView:Sprite,thumb:Sprite = null, track:Sprite = null, rect:Rectangle = null, upButton:SimpleButton = null, downButton:SimpleButton = null):void {
		if(contentView)
		{
			_containView = contentView;
			_containView.addChild(track);
			_containView.addChild(thumb);
		}
		if (thumb) {
			this.Thumb = thumb;
	...
	public static function initBaseScroll(contentView:Sprite=null,thumb:Sprite = null,track:Sprite = null,rect:Rectangle=null,upButton:SimpleButton = null,downButton:SimpleButton = null):baseScroll_taven {
		return new baseScroll_taven(contentView,thumb, track, rect, upButton, downButton);
	}
	public static function initBaseScrollObject(contentView:Sprite,_owern:Sprite,rect:Rectangle=null):baseScroll_taven {
		return new baseScroll_taven(contentView,_owern.getChildByName("Thumb") as Sprite, _owern.getChildByName("Track") as Sprite, rect,_owern.getChildByName("UpButton") as SimpleButton, _owern.getChildByName("DownButton") as SimpleButton);
	}
	...
	private function onStageMouseUp(evt:Event):void
	{
		...
		this.view.stage.removeEventListener(MouseEvent.MOUSE_UP, onStageMouseUp);
	}

	private function  onPanClick(evt:Event):void
	{
		...
		if(evt.type==MouseEvent.MOUSE_DOWN)
		{
			switch(evt.target)
			{
				case this.Thumb:
					...
					if(this.view.stage)
						this.view.stage.addEventListener(MouseEvent.MOUSE_UP, onStageMouseUp);
					break;
				case this.UpButton:
...
//		}
	...
	//鼠标模式
	public  function set buttonMode(_v:Boolean):void {
		this._buttonMode = _v;
	...
	protected  function scaleMode():void {
		if (this.rect&&this._autoScale) {
			...
			if (num < int(this.trackRect.height)) {
				if (this.Thumb.getChildByName("ThumbBg")) {
					this.Thumb.getChildByName("ThumbBg").height = num < this._scaleMinValue?this._scaleMinValue:num;
				}else{
					this.Thumb.height = num < this._scaleMinValue?this._scaleMinValue:num;
				}
				this.rect.height = this.trackRect.height - this.Thumb.height;
			}
		}
...
//				}
	...
	public function get scaleMinValue():int{
		return _scaleMinValue;
	}
}
}