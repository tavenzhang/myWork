 ...
 * 	
 * 	modifier: Abel Lee
 * 	说明：将传入的bitmapData设置为父类的bitmapData，并clone了一份给_originalBitmap
 * 	在处理完图片的9宫格拉伸后，将父类的bitmapData.dispose()
 * 	
 * 	调整：将传入的bitmapData.clone()后设置给父级
 *
 */
...
package {
	...
	public class ScaleBitmap extends Bitmap {
		...
		public function ScaleBitmap(bmpData : BitmapData = null, pixelSnapping : String = "auto", smoothing : Boolean = false) {
			// super constructor
			super(bmpData.clone(), pixelSnapping, smoothing);
