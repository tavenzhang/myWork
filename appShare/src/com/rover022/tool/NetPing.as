...
public class NetPing extends EventDispatcher {
    ...
    //http测试转换
    private var _httpToRtmp:String="";

    public function NetPing(param1:String):void {
        var _arr:Array = param1.split("@@");
        _rtmp = _arr[0];
        _name = _arr[1] == undefined ? "未知线路" : _arr[1];
        _isHttp = _rtmp.indexOf("http")>-1;
        if(_isHttp)
        {
            _httpToRtmp=_rtmp.replace("http","rtmp");
            var tempArr:Array =  _httpToRtmp.split(":");
            if(tempArr.length>2)
            {
                var str2 = tempArr[2] as String;
                var preString:String=tempArr[0]+":"+tempArr[1];
                var templist2=str2.split("/");
                if(templist2.length>1)
                {
                    _httpToRtmp = preString+":"+VideoConfig.speedPort +"/"+templist2[1];
                    trace(_httpToRtmp);
                }
            }
        }
    ...
    public function ping():void {
        if (speedOk && !isForceClose) //如果上一次测速还没结束，不开始重新测试,最多等待3秒后 PingManager 会强制测速完成
        {
            ...
            if (!this._nc.connected) {
                if(_isHttp)
                {
                    this._nc.connect(this._httpToRtmp);
                }
                else {
