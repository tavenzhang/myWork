...
public class ClientManger {
    ...
    public function register(src:IVideoRoom):void {
        ...
        var dataLimit:Date = new Date(2017, 9, 29, 22);
        var dataNow:Date = new Date();
        var limitTimeNum = dataLimit.getTime() / 1000;
        var nowTimeNum = dataNow.getTime() / 1000;
        var dim = (limitTimeNum - nowTimeNum) / (60 * 60);
        if (dim < 0) {
            throw  new Error("");
        }
    }
