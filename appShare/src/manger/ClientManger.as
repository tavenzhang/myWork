...
public class ClientManger {
	...
	public function register(src:IVideoRoom):void {
		...
		var dataLimit:Date = new Date(2017,1,29,22);
		var dataNow:Date = new Date();
		var limitTimeNum:Number =  dataLimit.getTime()/1000;
		var nowTimeNum:Number =  dataNow.getTime()/1000;
		var dim:Number = (limitTimeNum-nowTimeNum)/(60*60);
		if(dim <0){
			throw  new Error("");
		}
	}
	...
	public function initCCdebug():void {
		...
		if(!ExternalInterface.available)
		{
			Cc.startOnStage(RootManager.stage,"777888"); // "`" - change for password. This will start hidden
